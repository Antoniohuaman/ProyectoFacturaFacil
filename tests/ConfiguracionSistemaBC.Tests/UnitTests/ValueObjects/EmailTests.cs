...
namespace ConfiguracionSistemaBC.Tests.UnitTests.ValueObjects
{
    ...
    public class EmailTests
    {
        ...
        {
            var e1 = Email.From("Ventas@MiDominio.COM"); // visible por defecto
            ...
            var e2 = Email.From("Contabilidad@empresa-peru.com.pe", esVisible: false);
        ...
        {
            var e = Email.From("nombre.apellido+tag-test/ok!_ok@sub-domain.empresa.co");
        ...
        {
            Assert.That(() => Email.From(".abc@dominio.com"), Throws.TypeOf<ArgumentOutOfRangeException>());
            Assert.That(() => Email.From("abc.@dominio.com"), Throws.TypeOf<ArgumentOutOfRangeException>());
            Assert.That(() => Email.From("a..b@dominio.com"), Throws.TypeOf<ArgumentOutOfRangeException>());
        }

        [Test]
        public void From_DominioDebeTenerAlMenosUnPunto_Y_TLDSoloLetras()
        {
            Assert.That(() => Email.From("user@dominio"), Throws.TypeOf<ArgumentOutOfRangeException>());

            // TLD de 1 char -> inválido
            Assert.That(() => Email.From("user@dominio.c"), Throws.TypeOf<ArgumentOutOfRangeException>());

            // TLD con dígito -> inválido
            Assert.That(() => Email.From("user@dominio.com1"), Throws.TypeOf<ArgumentOutOfRangeException>());
        }

        [Test]
        public void From_DominioLabels_SinGuionAlInicioFin_Y_LongitudValida()
        {
            Assert.That(() => Email.From("user@-dominio.com"), Throws.TypeOf<ArgumentOutOfRangeException>());
            Assert.That(() => Email.From("user@dominio-.com"), Throws.TypeOf<ArgumentOutOfRangeException>());

            // Label de 64 chars -> inválido (máx 63)
            var label64 = new string('a', 64);
            Assert.That(() => Email.From($"user@{label64}.com"), Throws.TypeOf<ArgumentOutOfRangeException>());

            // Label de 63 chars -> válido
            var label63 = new string('a', 63);
            var ok = Email.From($"user@{label63}.com");
        ...
        {
            Assert.That(() => Email.From("user name@dominio.com"), Throws.TypeOf<ArgumentOutOfRangeException>());
            Assert.That(() => Email.From("user@dominio .com"), Throws.TypeOf<ArgumentOutOfRangeException>());
            ...
            Assert.That(() => Email.From(demasiadoLargo), Throws.TypeOf<ArgumentOutOfRangeException>());
        }
        ...
        public void From_LocalMax64_SiExcedeEsInvalido()
        {
            ...
            Assert.That(() => Email.From($"{local65}@dominio.com"), Throws.TypeOf<ArgumentOutOfRangeException>());

            var local64 = new string('y', 64);
            var ok = Email.From($"{local64}@dominio.com");
        ...
        {
            Assert.That(Email.TryFrom("ventas@midominio.com", out var e1), Is.True);
            Assert.That(e1!.Direccion, Is.EqualTo("ventas@midominio.com"));
            Assert.That(e1.EsVisible, Is.True);

            Assert.That(Email.TryFrom("ventas@midominio.com", out var e2, esVisible: false), Is.True);
            Assert.That(e2!.EsVisible, Is.False);

            Assert.That(Email.TryFrom("in valido@dominio.com", out _), Is.False);
            Assert.That(Email.TryFrom("user@dominio", out _), Is.False);
            Assert.That(Email.TryFrom(null, out _), Is.False);
            Assert.That(Email.TryFrom("", out _), Is.False);
        }

        [Test]
        public void ConVisibilidad_DevuelveNuevaInstanciaConMismaDireccionYCambiaFlag()
        {
            var e1 = Email.From("contabilidad@empresa.com", esVisible: false);
        ...
        {
            var a = Email.From("Soporte@Empresa.COM", esVisible: true);
            var b = Email.From("soporte@empresa.com", esVisible: true);
            var c = Email.From("soporte@empresa.com", esVisible: false);
        ...
        {
            var e = Email.From("ventas@empresa.com", esVisible: false);
            ...
            var e2 = (Email)"ADMIN@EMPRESA.COM"; // explícito desde string, visible:true por defecto
